...
class Task1 extends Module {
    ...
    val arb_prior = Module(new Arbiter(UInt(),4))

        arb_prior.io.in(0) <> queue_0
        arb_prior.io.in(1) <> queue_1
        arb_prior.io.in(2) <> queue_2
        arb_prior.io.in(3) <> queue_3

        arb_prior.io.out.ready := 1.B
        
        when(io.write_en) {
            memory.write(io.writeaddr, (arb_prior.io.out).asUInt)
        }
